...
class GatedDeltaProductMixer(SSMDynamicInferenceMixin, MegatronModule):
    ...
    def ssm_prefill(
        ...
        # static-batching window with F.pad instead of one row per request. The
        # carrying variant is required under chunked prefill: a prompt's final
        # chunk can be shorter than d_conv, and deriving the state from that slice
        # alone would zero-fill taps the next decode step still needs.
