...
class GatedDeltaProductMixer(SSMDynamicInferenceMixin, MegatronModule):
    ...
    def ssm_decode(
        ...
        # filled batch match an eager run. Mirrors `MambaMixer.ssm_decode`, which
        # routes static and dynamic batching through the same kernel.
        VKQ = causal_conv1d_update_triton(
            VKQ,
            conv_state,
    ...
    def ssm_prefill(
        ...
        if extract_intermediates:
            core_attn_out, chunk_states = chunk_scan_result
        else:
