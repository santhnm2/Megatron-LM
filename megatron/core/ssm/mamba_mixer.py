...
class MambaMixer(MegatronModule):
    ...
    def _ssm_prefill(
        ...
        Args:
            ...
            intermediate_real_count: ``int32[1]`` GPU tensor holding the number
                of meaningful entries in the intermediate buffers this step.
                Read inside the Triton scatter kernels so padded slots cost
                nothing.
        ...
        if is_dynamic_batching:
            ...
            # Extraction is enabled when the slot allocator wired buffers in
            # via the caller; the chunk scan returns raw states so our Triton
            # kernel can do a fused gather+conditional-scatter, skipping the
            # densely-written intermediate tensor and the padded-slot writes
            # entirely. Padded slots cost nothing — they exit on the
            # `pid_slot >= real_count` guard inside the kernel.
            ...
            if extract_intermediates:
                ...
                # raw_ssm_states[chunk_indices[i]] and write into
                # intermediate_ssm_out[i], only for i < real_count.
                scatter_intermediate_ssm(
                    raw_ssm_states,
                ...
                # Same pattern for conv: gather a length-d_conv window ending
                # at abs_positions[i] (clamped into the valid token range)
                # from xBC_pre_conv and scatter (transposed) into
                # intermediate_conv_out[i], only for i < real_count.
                scatter_intermediate_conv(
                    xBC_pre_conv,
