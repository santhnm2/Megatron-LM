...
Forward-only fork of `chunk_gated_delta_product`. The stage sequence and the
math are upstream's, unchanged:

1. Interleave the log decays across the Householder copies and take the
...
from .common import CHUNK_SIZE, l2norm_fwd, prepare_chunk_indices
...
def chunk_gated_delta_product_varlen(
    ...
    # argument contiguous. The kernels index with raw pointer arithmetic and
