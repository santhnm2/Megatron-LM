...
Forward-only fork. Gated Delta Product decode reaches this kernel by folding
the `M` Householder copies into the sequence dimension, so a single decode
token becomes an `M`-length sequence with the query placed on the last copy and
the decay on the first; the caller slices the answer back out.

The gate variants `gk` and `gv` that upstream carries are dropped: Gated Delta
Product only ever passes the scalar gate `g`.
"""
...
def fused_recurrent_gated_delta_rule_update(
    ...
    # Upstream wraps its entry point in `input_guard`, which forces every tensor
    # argument contiguous. The kernel indexes with raw pointer arithmetic and
    # would read garbage from a strided input, so reproduce that here.
