...
#
# This is the *inference-only* fork of FLA's fused recurrent gated delta rule,
# used by the Gated Delta Product decode path. Training and prefill continue to
# call the pip-installed `flash-linear-attention` kernels; only the dynamic
# batching decode step routes here. The fork exists for the same reasons as the
# Mamba2 kernel fork in the sibling `mamba2` package:
#
#   1. Slot-indexed recurrent state. The recurrent state lives in a persistent
#      per-request cache owned by `DynamicInferenceContext`; this kernel reads
#      and writes it *in place* at the rows named by `state_indices`, so no
#      gather of the initial state and no scatter of the final state is needed.
#   2. CUDA-graph friendliness. `state_indices` may contain `-1` for padding
#      requests (batch shapes are rounded up to a captured graph size). Padded
#      rows write zeros to the output and touch no state, so a replayed graph
#      with a partially filled batch produces the same result as an eager run
#      with only the real requests.
#
# Deliberately dropped relative to upstream (not needed for decode):
# `gk`/`gv` decays, varlen `cu_seqlens`, the transposed state layout, and
# the autograd wrapper (the backward pass is unimplemented upstream anyway).
