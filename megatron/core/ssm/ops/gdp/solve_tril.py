...
Only the 64x64 path is vendored. Upstream dispatches on a chunk length of 16,
32 or 64; the Gated Delta Product kernels are wired to 64 throughout (see
`CHUNK_SIZE`), so the 16x16 and 32x32 entry points are unreachable from here.
...
def solve_tril(
    ...
    assert A.shape[-1] == 64, "this fork only vendors the 64x64 inverse"
