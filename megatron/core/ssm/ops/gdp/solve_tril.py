...
Forked from `merge_16x16_to_64x64_inverse_kernel`. Changes: only the 64x64
merge path is kept (the forked GDP prefill always runs at chunk size 64), the
TMA descriptor path is dropped (TMA descriptors are built per launch and are
not what we want under graph capture), no autotuning, `ieee` dot precision is
fixed rather than autotuned over, and `chunk_indices` comes from the caller.
"""
...
def merge_16x16_to_64x64_inverse_kernel(
    ...
    """Invert `I + A` blockwise: four 16x16 diagonal inverses, then merge."""
...
def solve_tril(
    ...
    """Compute `(I + A)^-1` for a strictly lower-triangular `A` of shape
    `[B, T, H, 64]` (one 64x64 block per chunk row)."""
