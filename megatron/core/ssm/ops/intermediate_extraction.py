...
These kernels replace the two-step ``states[indices]`` (gather) + ``.copy_()``
...
This is CUDA-graph safe: the launch grid is sized at capture time to the
maximum possible slot count, but per-program execution is data-conditional on
the runtime ``real_count`` value, so padded slots cost almost nothing.
"""
...
def _scatter_intermediate_ssm_kernel(
    ...
    Grid: ``(max_count, ceil(state_flat / BLOCK_N))``.

    Each program owns one (slot, column-block) pair. Programs with
    ``pid_slot >= real_count`` exit immediately, so padded slots produce no
    HBM traffic.
    """
...
def scatter_intermediate_ssm(
    ...
    real_count_gpu: Tensor,
    out: Tensor,
) -> None:
    """Gather rows of ``states`` indexed by ``chunk_indices`` and scatter into ``out``,
    only for the first ``real_count_gpu`` slots.

    Args:
        states: ``(num_chunks, *ssm_state_shape)`` — output of the chunk scan
            for a single layer.
        chunk_indices: ``(max_count,)`` int64 — per-slot gather index.
        real_count_gpu: ``int32[1]`` GPU tensor with the runtime real count.
        out: ``(max_count, *ssm_state_shape)`` — destination scratch slice for
            this layer.
    """
    ...
    assert out.shape[1:] == states.shape[1:], (
        f"per-slot shape mismatch: out {tuple(out.shape[1:])} "
        f"vs states {tuple(states.shape[1:])}"
    )
    ...
    # The grid follows the per-step view length (chunk_indices.numel()), which
    # is bounded above by out.shape[0] (the full scratch pool); programs with
    # pid_slot >= real_count exit immediately inside the kernel.
    n_slots = int(chunk_indices.numel())
    if n_slots == 0:
        return
    if n_slots > out.shape[0]:
        raise ValueError(
            f"chunk_indices length {n_slots} exceeds scratch capacity {out.shape[0]}"
        )
    ...
    # Both tensors must be contiguous along their per-slot dims for the
    # row-stride math below; the engine pre-allocates them so this is always
    # the case in production, but assert to fail loudly in tests.
...
def _scatter_intermediate_conv_kernel(
    ...
    abs_positions_ptr,      # [max_count] int32: extraction center per slot
    ...
    Reads a window ``[abs_pos - D_CONV, abs_pos)`` from ``src_ptr`` (clamped
    into ``[0, seq_len)``) and writes it transposed into
    ``out[slot, :, :]`` of shape ``(conv_dim, D_CONV)``. The transpose is folded
    into the write pattern to match the slot allocator's storage layout.
    """
    pid_slot = tl.program_id(0)
    ...
    # Pre-compute the per-slot destination base. The write layout is
    # out[slot, c, j] where c is outer and j is inner (contiguous), so for a
    # fixed c the D_CONV values are stride-1.
    slot_base = pid_slot.to(tl.int64) * out_slot_stride

    # Unrolled across the small D_CONV dimension.
    for j in tl.static_range(D_CONV):
        p_raw = abs_pos - D_CONV + j
        # Clamp into [0, seq_len - 1]. For real slots this is a no-op; the
        # clamp is defensive for any future caller that doesn't filter
        # padded slots out via real_count.
        p = tl.maximum(0, tl.minimum(p_raw, seq_len - 1))

        src = src_ptr + p.to(tl.int64) * src_stride_s + c_idxs.to(tl.int64) * src_stride_c
        # out[slot, c, j] linear offset: slot * conv_dim * D_CONV + c * D_CONV + j.
...
def scatter_intermediate_conv(
    src: Tensor,
    abs_positions: Tensor,
    real_count_gpu: Tensor,
    out: Tensor,
    d_conv: int,
) -> None:
    """Gather length-``d_conv`` conv windows from ``src`` at positions given by
    ``abs_positions`` and scatter (transposed) into ``out``, only for the first
    ``real_count_gpu`` slots.

    Args:
        src: ``(batch, seq_len, conv_dim)`` — pre-conv xBC tensor. Batch is
            assumed to be 1 (inference); only batch index 0 is read.
        abs_positions: ``(max_count,)`` int32 — extraction-window end position
            per slot (the window is ``[pos - d_conv, pos)``).
        real_count_gpu: ``int32[1]`` GPU tensor with the runtime real count.
        out: ``(max_count, conv_dim, d_conv)`` — destination scratch slice for
            this layer.
    ...
    if n_slots > out.shape[0]:
        raise ValueError(
            f"abs_positions length {n_slots} exceeds scratch capacity {out.shape[0]}"
        )
