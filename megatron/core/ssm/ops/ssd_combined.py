...
def _mamba_chunk_scan_combined_fwd(
    ...
    if return_raw_states:
        # Caller does any chunk-boundary state extraction itself (e.g. via a
        # fused gather+scatter Triton kernel) to avoid the cost of materializing
        # a max_count-sized intermediate tensor.
...
def mamba_chunk_scan_combined_varlen(
    ...
    Argument:
        ...
        return_raw_states: If True, returns ``(varlen_states, raw_states)`` where
            ...
            chunk-boundary state tensor. The caller is expected to do any
            extraction (e.g. a fused Triton gather+scatter) directly on
            ``raw_states``, bypassing the dense gather that would otherwise
            materialize a max_count-sized intermediate tensor.
